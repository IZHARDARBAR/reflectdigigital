...
const BUBBLE_CONFIG = [
  "bottom-10 left-10 w-12 h-12 sm:w-16 sm:h-16 animate-float-up-1",
  "bottom-20 right-4 sm:right-20 w-10 h-10 sm:w-12 sm:h-12 animate-float-up-2",
  "bottom-32 left-1/4 w-8 h-8 sm:w-10 sm:h-10 animate-float-up-3",
  "bottom-16 right-1/4 sm:right-1/3 w-10 h-10 sm:w-14 sm:h-14 animate-float-up-4",
  "bottom-40 left-3/4 w-6 h-6 sm:w-8 sm:h-8 animate-float-up-5",
  "bottom-24 left-1/3 w-7 h-7 sm:w-9 sm:h-9 animate-float-up-6",
  "bottom-28 right-1/4 w-8 h-8 sm:w-11 sm:h-11 animate-float-up-7",
  "bottom-36 left-2/3 w-5 h-5 sm:w-7 sm:h-7 animate-float-up-8",
  "bottom-12 left-12 sm:left-16 w-8 h-8 sm:w-13 sm:h-13 animate-float-up-9",
  "bottom-48 right-8 sm:right-16 w-4 h-4 sm:w-6 sm:h-6 animate-float-up-10",
  "bottom-22 left-3/4 w-7 h-7 sm:w-10 sm:h-10 animate-float-up-11",
  "bottom-38 right-3/4 w-5 h-5 sm:w-8 sm:h-8 animate-float-up-12",
  "bottom-15 left-8 sm:left-12 w-4 h-4 sm:w-7 sm:h-7 animate-float-up-13",
  "bottom-52 right-16 sm:right-32 w-7 h-7 sm:w-11 sm:h-11 animate-float-up-14",
  "bottom-26 left-40 sm:left-60 w-6 h-6 sm:w-9 sm:h-9 animate-float-up-15",
  "bottom-44 left-32 sm:left-44 w-3 h-3 sm:w-5 sm:h-5 animate-float-up-16",
];
...
// ==========================================
// 2. SUB-COMPONENTS (For Readability)
// ==========================================

// --- Stat Card Component ---
const StatCard = ({ title, desc, amount, amountColor, Icon, iconColor }) => (
  <div className="bg-[#1C1C1E] rounded-2xl sm:rounded-3xl p-6 sm:p-8 lg:p-10 flex flex-col justify-between border border-gray-800 min-h-[350px] sm:min-h-[450px]">
    ...
    <div className="flex justify-between items-start">
      ...
      <div className={iconColor}>
        <Icon size={40} className="sm:w-16 sm:h-16 lg:w-20 lg:h-20" />
      </div>
    </div>
    ...
    <div className={`text-4xl sm:text-5xl lg:text-6xl xl:text-7xl font-bold ${amountColor} flex items-start gap-1 sm:gap-2`}>
      <span className={`${amountColor} text-2xl sm:text-3xl lg:text-4xl xl:text-5xl mt-1 sm:mt-2 lg:mt-3`}>$</span>{" "}
      {amount}
    </div>
  </div>
);
...
const HistoryCard = ({ item }) => {
  return (
    <div className="bg-[#1C1C1E] rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 border border-gray-800 h-auto flex flex-col justify-between">
      {/* Top Section */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-6">
        <div className="flex items-center gap-4 sm:gap-6">
          {/* Logo Size */}
          <div className={`w-12 h-12 sm:w-16 sm:h-16 lg:w-20 lg:h-20 ${item.logoColor} rounded-lg sm:rounded-xl border border-gray-600 shadow-md`}></div>
          <div>
            {/* Text Size */}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 lg:gap-6 md:gap-8">
        <StatDetail label="BRAND VALUE" value={item.brandValue} />
        <StatDetail label="COMMISSIONS" value={item.commissions} />
        <StatDetail label="PROFITS" value={item.profits} />
        <StatDetail label="REBATE" value={item.rebate} />
      </div>
    </div>
  );
};

// Helper for HistoryCard Stats
...
// --- Interbrand Popup Component ---
const InterbrandPopup = ({ onClose, onConfirm }) => (
...
export default function Start() {
  ...
  // Handle Handlers
  const handleStartWork = () => setShowInterbrandPopup(true);
  ...
  return (
    <div className="min-h-screen bg-black text-white overflow-x-hidden font-sans">
      ...
      <div 
        ...
        style={{ backgroundImage: "url('/startbg.png')" }}
      >
        {/* Bubbles Animation Layer */}
        {BUBBLE_CONFIG.map((className, index) => (
          <img 
            key={index} 
            src="/bubble.png" 
            alt="" 
      ...
      <div className="bg-black py-6 sm:py-10">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {/* 1. Balance Stats Grid */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-6 sm:mb-8">
            <StatCard 
              title="Total Account Balance"
              desc="Includes our daily profits, which makes it super easy to see how we're doing overall"
              amount="32,148.46"
              amountColor="text-white"
              Icon={FaCoins}
              iconColor="text-[#FFB37C]"
            />
            <StatCard 
              title="Insufficient Balance"
              desc="Amount required to deposit in order to proceed with the next task"
              amount="16,961.83"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-8 sm:mb-16">
            <StatCard 
              ...
              desc="Your profits will be refreshed daily based on the tasks you complete"
              amount="0.00"
              amountColor="text-white"
              Icon={FaChartBar}
              iconColor="text-[#A855F7]"
            />
            {/* Empty div for grid balance on large screens */}
      ...
      {showInterbrandPopup && (
        <InterbrandPopup 
          onClose={() => setShowInterbrandPopup(false)} 
      ...
      <style jsx>{`
        ...
        .animate-float-up-1 { animation: floatUp 8s linear infinite; animation-delay: 0s; }
        .animate-float-up-2 { animation: floatUp 10s linear infinite; animation-delay: 1s; }
        .animate-float-up-3 { animation: floatUp 12s linear infinite; animation-delay: 2s; }
        .animate-float-up-4 { animation: floatUp 9s linear infinite; animation-delay: 3s; }
        .animate-float-up-5 { animation: floatUp 11s linear infinite; animation-delay: 4s; }
        .animate-float-up-6 { animation: floatUp 7s linear infinite; animation-delay: 5s; }
        .animate-float-up-7 { animation: floatUp 13s linear infinite; animation-delay: 6s; }
        .animate-float-up-8 { animation: floatUp 8s linear infinite; animation-delay: 7s; }
        .animate-float-up-9 { animation: floatUp 8s linear infinite; animation-delay: 8s; }
        .animate-float-up-10 { animation: floatUp 10s linear infinite; animation-delay: 9s; }
        .animate-float-up-11 { animation: floatUp 12s linear infinite; animation-delay: 10s; }
        .animate-float-up-12 { animation: floatUp 9s linear infinite; animation-delay: 11s; }
        .animate-float-up-13 { animation: floatUp 11s linear infinite; animation-delay: 12s; }
        .animate-float-up-14 { animation: floatUp 7s linear infinite; animation-delay: 13s; }
        .animate-float-up-15 { animation: floatUp 13s linear infinite; animation-delay: 14s; }
        .animate-float-up-16 { animation: floatUp 8s linear infinite; animation-delay: 15s; }
      `}</style>
    </div>
  );
}